...
const LISTINGS_QUERY_KEY = ["agent-portal", "marketplace", "listings"] as const;
const PUBLISHABLE_QUERY_KEY = ["agent-portal", "marketplace", "publishable-plots"] as const;
const CONVERSATIONS_QUERY_KEY = ["agent-portal", "marketplace", "conversations"] as const;
    ...
    <div>
      ...
      <Tabs
        ...
        items={[
                ...
                {(listings ?? []).map((listing) => (
                  <Col key={listing.id} xs={24} sm={12} md={8} lg={6}>
                    <MarketplaceListingCard
                      listing={listing}
                      canManagePhotos={listing.publishedBy === claims?.agentId}
          ...
          {
            key: "publish",
            label: "Publish a Plot",
            children: (
              <Space direction="vertical" style={{ width: "100%" }}>
                <Typography.Paragraph type="secondary">
                  Select one of your available plots to list on the marketplace.
                ...
                {(publishablePlots ?? []).length === 0 ? (
                  <Empty description="No plots available to publish. Marketplace publishing may not be enabled for your account, or no plots are currently visible to you." />
                ) : null}
              </Space>
            ),
          },
          ...
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, padding: 16 }}>
            {activeConversation.messages.length === 0 ? (
              ...
              activeConversation.messages.map((entry, index) => {
                const isMine = entry.senderRole === "Agent";
                return (
                  <div
                    key={index}
                    style={{
